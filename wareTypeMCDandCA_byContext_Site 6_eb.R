...
#### 8. Remove specific ware types (if you must) ####
...
# want included
wareTypeData1 <-filter(wareTypeData_Unit, !Ware %in% c('American Stoneware', 
...
wareByUnitT <- wareTypeData1 %>% group_by(Ware,unit) %>% 
  summarise(count = sum(Quantity)) %>%
...
#### 11. Seriation diagram based on MCDs ####
...
#### 14. Now let's try some Correspondence Analysis ####
# You need to decide if you want to use the same data that 
# went into the MCD analysis (WareByUnitT2), or the data 
# will all the ware types. To chose, commment out one of these two lines:
...
p <- ggplot(data=inertia , aes(x= 1:length(Inertia), y=Inertia)) +
  ...
  labs( title="Site 6 CA Scree Plot", x="Dimension", y='Proportion of Inertia' ) +
...
#### 15. Compare MCD and CA dim scores
...
#### 16. Histogram of Dim 1 scores for Phasing
...
p5 <- ggplot(dim1ForHist, aes(x = dim1)) +
  ...
  theme(plot.title = element_text(hjust = 0.5))+
  labs(title="Site6", x="Dimension 1", y="Density") +
...
MCDByPhase
#Export data
#write.csv(CA_MCD_Phase, file='CA_MCD_Phase_Site6.csv')


# BlueMCD By Dim1 plot by Phase
# This one uses DAACS Website colors library(viridis)
...
p6 <- ggplot(CA_MCD_Phase,aes(x = Dim1,y = blueMCD, fill= factor(Phase))) +
  ...
  #geom_text_repel(aes(label= unit), cex=4) +
  theme(plot.title = element_text(hjust = 0.5))+
  labs(title="Site6", x="Dimension 1", y="BLUE MCD")
p6
