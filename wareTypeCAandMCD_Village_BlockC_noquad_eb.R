...
#### 8. Tranpose the data for the MCD and CA ####
...
# 9.2  We may also want to enforce a sample size cut off on the MCD analysis.
...
EstimateMCD<- function(unitData,typeData){
  ...
  repUnitData <- mergedUnitData[rep(rownames(mergedUnitData),mergedUnitData$count),c(1,3:6)]
  str(repUnitData)
...
p <- ggplot(data=inertia , aes(x= 1:length(Inertia), y=Inertia)) +
  ...
  labs( title="Morne Patate CA Scree Plot", x="Dimension", y='Proportion of Inertia' ) +
...
#ggsave("MP_Figure1Dim1Dim2_2018cxt.png", p1, width=10, height=7.5, dpi=300)
...
# save the plot for website chronology page/presentations
# ggsave("MP_Histogram.png", p5a, width=10, height=7.5, dpi=300)
